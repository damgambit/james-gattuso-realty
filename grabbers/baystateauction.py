...
with open("./csv/baystateauction.csv","w")as export:
    ...
    f = open('./csv/baystateauction.csv', 'r')
    reader = csv.reader(f)
    ...
    for row in reader:
        ...
        if 'December' in row[2]:
            date_to_save = datetime.datetime.strptime(str(row[2]) + ' 2017', '%B %d %Y')
        else: 
            date_to_save = datetime.datetime.strptime(str(row[2]) + ' 2018', '%B %d %Y')
