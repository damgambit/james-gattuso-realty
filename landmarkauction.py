...
def parser(self,headers,writer):
    ...
    print([Date,Time,encoding(Status),encoding(Address),encoding(City),encoding(State),encoding(Zipcode)])

    zipcode = encoding(Zipcode)

    if 'MA' in zipcode:
        State = 'MA'
        zipcode = zipcode[5:]

    c = LandMarkAuction.objects.get_or_create(date=Date,
                                                  time=Time,
...
with open('./csv_file/landmarkauction.csv','w')as export:
